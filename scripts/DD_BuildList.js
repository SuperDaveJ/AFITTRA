...
function evalMatches() {
	for (var i=0; i<nObj; i++) {
		...
		if (i<9) {
		  tempX = eval("dd.elements.drag_"+(i+1)+".x");
		  tempY = eval("dd.elements.drag_"+(i+1)+".y");
		  if ( (tempX > tX) && (tempX < (tX + tW)) && (tempY > tY) && (tempY < (tY + tH)) ) {
			  arrUser[i] = eval("dd.elements.drag_"+(i+1)+".name");
		  }
		} else {
			//10th object and up
			charTemp = String.fromCharCode(88+i);
			//return a letter from Char Code 97=a, 98=b, etc.
		  	tempX = eval("dd.elements.drag_" + charTemp + ".x");
		  	tempY = eval("dd.elements.drag_" + charTemp + ".y");
		  	if ( (tempX > tX) && (tempX < (tX + tW)) && (tempY > tY) && (tempY < (tY + tH)) ) {
			  	arrUser[i] = eval("dd.elements.drag_" + charTemp + ".name");
		  	}
		}
	}
}
	...
	if (uCorrect == nObj) {
		...
		if (TriesUser < TriesLimit) {
			...
			for (var i=nCorrect+1; i<=nObj; i++) {
				...
				if ( i < 10 ) {
				  eval("dd.elements.drag_" + i + ".moveTo(dd.elements.drag_" + i + ".defx, dd.elements.drag_" + i + ".defy)");
				} else {
				  charTemp = String.fromCharCode(87+i);
			...
			for (var i=1; i<=nCorrect; i++) {
				...
				if ( i < 10 ) {
				  eval("dd.elements.drag_" + i + ".moveTo(dd.elements.target" + i + ".defx, dd.elements.target" + i +".defy)");
				} else {
				  charTemp = String.fromCharCode(87+i);
				  eval("dd.elements.drag_" + charTemp + ".moveTo(dd.elements.target" + i + ".defx, dd.elements.target" + i +".defy)");
				}
			}
