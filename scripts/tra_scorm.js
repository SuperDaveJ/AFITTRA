// JavaScript Document for AFIT - TRA
...
function gotoPage(direction, pgURL) {
	...
	if (contentFrame.isNextDisabled == false) { //if next button not disabled
	  if (direction == "f") {
...
function startCourse() {
	...
	if (inLMS == true) {
		...
		if ( Get_Cookie("userData_tra") ) {
			getSuspendData();
		} else {
			Set_Cookie("userData_tra", initialStatus, 60, "/", "", "");
			strAllLessonStatus = initialStatus;
		}
		if ( Get_Cookie("userBookmark_tra") ) {
			bookmark = Get_Cookie("userBookmark_tra");
			if ( bookmark.indexOf(mName) >= 0 ) bookmark = "";
		}
	}

	if (bookmark != "" && getLesson()==0) {
...
function exitCourse() {
	if ( exitPageStatus != true ) {
		...
		if ( inLMS == true ) {
			...
			Set_Cookie("userBookmark_tra", contentFrame.location.href, 60, "/", "", "");
		}
		window.close();
	}
}
...
//This function is for module 3 - test only
...
function updateSuspendData() {
	...
	if (inLMS == true) {
		...
		Set_Cookie("userData_tra", strTemp, 60, "/", "", "");
	}
}
...
function getSuspendData() {
	if (inLMS == true) {
		...
		strTemp = Get_Cookie("userData_tra");
	}
	if ( (strTemp != "") && (typeof(strTemp) != "undefined") ) {
		arrTemp = new Array();
